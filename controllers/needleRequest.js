...
function apiRequest(method, url, data, options={}){
    ...
    let reff = genReff()
    logging.info(`[FLIP-API][REQ][OUT] REFF: ${reff} ${method} ${baseUrl} ${JSON.stringify(data)} ${JSON.stringify(options)}`)
    return new Promise(function(resolve, reject) {
        needle.request(method, baseUrl, data, options, function(err, resp, body) {
            logging.info(`[FLIP-API][RES][IN] REFF: ${reff} ${resp.statusCode} ${JSON.stringify(body)}`)
